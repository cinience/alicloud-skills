---
name: aliyun-modelstudio-qwen-ocr
...
```bash
mkdir -p output/aliyun-modelstudio-qwen-ocr
python -m py_compile skills/ai/multimodal/aliyun-modelstudio-qwen-ocr/scripts/prepare_ocr_request.py && echo "py_compile_ok" > output/aliyun-modelstudio-qwen-ocr/validate.txt
```

Pass criteria: command exits 0 and `output/aliyun-modelstudio-qwen-ocr/validate.txt` is generated.

## Output And Evidence

- Save request payloads, selected OCR task name, and normalized output expectations under `output/aliyun-modelstudio-qwen-ocr/`.
...
```bash
python skills/ai/multimodal/aliyun-modelstudio-qwen-ocr/scripts/prepare_ocr_request.py \
...
- Default output: `output/aliyun-modelstudio-qwen-ocr/request.json`
