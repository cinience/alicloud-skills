---
name: aliyun-modelstudio-qwen-image
...
```bash
mkdir -p output/aliyun-modelstudio-qwen-image
python -m py_compile skills/ai/image/aliyun-modelstudio-qwen-image/scripts/generate_image.py && echo "py_compile_ok" > output/aliyun-modelstudio-qwen-image/validate.txt
```

Pass criteria: command exits 0 and `output/aliyun-modelstudio-qwen-image/validate.txt` is generated.

## Output And Evidence

- Write generated image URLs, prompts, and metadata to `output/aliyun-modelstudio-qwen-image/`.
...
```bash
curl -L -o output/aliyun-modelstudio-qwen-image/images/preview.png "<IMAGE_URL_FROM_RESPONSE>" && open output/aliyun-modelstudio-qwen-image/images/preview.png
```

Local helper script (JSON request -> image file):

```bash
python skills/ai/image/aliyun-modelstudio-qwen-image/scripts/generate_image.py \\
  --request '{"prompt":"a studio product photo of headphones","size":"1024*1024"}' \\
  --output output/aliyun-modelstudio-qwen-image/images/headphones.png \\
  --print-response
```
...
- Default output: `output/aliyun-modelstudio-qwen-image/images/`
...
- For edit workflows, use `skills/ai/image/aliyun-modelstudio-qwen-image-edit/`.
