---
name: aliyun-modelstudio-zimage-turbo
...
```bash
python skills/ai/image/aliyun-modelstudio-zimage-turbo/scripts/generate_image.py \
...
```bash
mkdir -p output/aliyun-modelstudio-zimage-turbo
for f in skills/ai/image/aliyun-modelstudio-zimage-turbo/scripts/*.py; do
  python3 -m py_compile "$f"
done
echo "py_compile_ok" > output/aliyun-modelstudio-zimage-turbo/validate.txt
```

Pass criteria: command exits 0 and `output/aliyun-modelstudio-zimage-turbo/validate.txt` is generated.

## Output And Evidence

- Save artifacts, command outputs, and API response summaries under `output/aliyun-modelstudio-zimage-turbo/`.
