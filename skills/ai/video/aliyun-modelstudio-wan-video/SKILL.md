---
name: aliyun-modelstudio-wan-video
...
```bash
mkdir -p output/aliyun-modelstudio-wan-video
python -m py_compile skills/ai/video/aliyun-modelstudio-wan-video/scripts/generate_video.py && echo "py_compile_ok" > output/aliyun-modelstudio-wan-video/validate.txt
```

Pass criteria: command exits 0 and `output/aliyun-modelstudio-wan-video/validate.txt` is generated.

## Output And Evidence

- Save task IDs, polling responses, and final video URLs to `output/aliyun-modelstudio-wan-video/`.
...
- Default output: `output/aliyun-modelstudio-wan-video/videos/`
