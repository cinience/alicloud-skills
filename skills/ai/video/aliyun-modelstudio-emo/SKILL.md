...
```bash
mkdir -p output/aliyun-modelstudio-emo
python -m py_compile skills/ai/video/aliyun-modelstudio-emo/scripts/prepare_emo_request.py && echo "py_compile_ok" > output/aliyun-modelstudio-emo/validate.txt
```

Pass criteria: command exits 0 and `output/aliyun-modelstudio-emo/validate.txt` is generated.

## Output And Evidence

- Save normalized request payloads, detection boxes, and task polling snapshots under `output/aliyun-modelstudio-emo/`.
...
```bash
python skills/ai/video/aliyun-modelstudio-emo/scripts/prepare_emo_request.py \
...
- Default output: `output/aliyun-modelstudio-emo/request.json`
