"""HappyHorse 1.0 Text-to-Video generation via DashScope HTTP API.

Usage:
    python t2v_happyhorse.py --prompt "a cat running" --duration 5 --resolution 720P
    python t2v_happyhorse.py --prompt "..." --ratio 9:16 --duration 10
"""
...
OUTPUT_DIR = os.getenv("OUTPUT_DIR", "output/aliyun-happyhorse-t2v/videos")
...
def create_task(
    ...
    """Submit an async t2v task. Returns task_id."""
    payload: dict = {
        "model": MODEL,
        "input": {"prompt": prompt},
        "parameters": {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="HappyHorse 1.0 Text-to-Video")
    ...
    print(f"Creating t2v task with model={MODEL}...")
    task_id = create_task(
        prompt=args.prompt,
