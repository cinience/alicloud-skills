---
name: aliyun-modelstudio-tts-realtime
...
```bash
.venv/bin/python skills/ai/audio/aliyun-modelstudio-tts-realtime/scripts/realtime_tts_demo.py \
...
```bash
mkdir -p output/aliyun-modelstudio-tts-realtime
for f in skills/ai/audio/aliyun-modelstudio-tts-realtime/scripts/*.py; do
  python3 -m py_compile "$f"
done
echo "py_compile_ok" > output/aliyun-modelstudio-tts-realtime/validate.txt
```

Pass criteria: command exits 0 and `output/aliyun-modelstudio-tts-realtime/validate.txt` is generated.

## Output And Evidence

- Save artifacts, command outputs, and API response summaries under `output/aliyun-modelstudio-tts-realtime/`.
