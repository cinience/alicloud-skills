---
name: aliyun-modelstudio-asr-realtime
...
```bash
mkdir -p output/aliyun-modelstudio-asr-realtime
python -m py_compile skills/ai/audio/aliyun-modelstudio-asr-realtime/scripts/prepare_realtime_asr_request.py && echo "py_compile_ok" > output/aliyun-modelstudio-asr-realtime/validate.txt
```

Pass criteria: command exits 0 and `output/aliyun-modelstudio-asr-realtime/validate.txt` is generated.

## Output And Evidence

- Save session payloads and response samples under `output/aliyun-modelstudio-asr-realtime/`.
...
```bash
python skills/ai/audio/aliyun-modelstudio-asr-realtime/scripts/prepare_realtime_asr_request.py \
  --output output/aliyun-modelstudio-asr-realtime/request.json
```
...
- If you only have recorded files, use `skills/ai/audio/aliyun-modelstudio-asr/` instead.
