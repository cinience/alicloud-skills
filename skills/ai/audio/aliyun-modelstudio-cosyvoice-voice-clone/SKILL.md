---
name: aliyun-modelstudio-cosyvoice-voice-clone
...
```bash
python skills/ai/audio/aliyun-modelstudio-cosyvoice-voice-clone/scripts/prepare_cosyvoice_clone_request.py \
...
```bash
mkdir -p output/aliyun-modelstudio-cosyvoice-voice-clone
for f in skills/ai/audio/aliyun-modelstudio-cosyvoice-voice-clone/scripts/*.py; do
  python3 -m py_compile "$f"
done
echo "py_compile_ok" > output/aliyun-modelstudio-cosyvoice-voice-clone/validate.txt
```

Pass criteria: command exits 0 and `output/aliyun-modelstudio-cosyvoice-voice-clone/validate.txt` is generated.

## Output And Evidence

- Save artifacts, command outputs, and API response summaries under `output/aliyun-modelstudio-cosyvoice-voice-clone/`.
