...
```bash
mkdir -p output/aliyun-modelstudio-emoji
python -m py_compile skills/ai/video/aliyun-modelstudio-emoji/scripts/prepare_emoji_request.py && echo "py_compile_ok" > output/aliyun-modelstudio-emoji/validate.txt
```

Pass criteria: command exits 0 and `output/aliyun-modelstudio-emoji/validate.txt` is generated.

## Output And Evidence

- Save normalized request payloads, detected face boxes, selected template ID, and task polling snapshots under `output/aliyun-modelstudio-emoji/`.
...
```bash
python skills/ai/video/aliyun-modelstudio-emoji/scripts/prepare_emoji_request.py \
...
- Default output: `output/aliyun-modelstudio-emoji/request.json`
