---
name: aliyun-modelstudio-livetranslate
...
```bash
mkdir -p output/aliyun-modelstudio-livetranslate
python -m py_compile skills/ai/audio/aliyun-modelstudio-livetranslate/scripts/prepare_livetranslate_request.py && echo "py_compile_ok" > output/aliyun-modelstudio-livetranslate/validate.txt
```

Pass criteria: command exits 0 and `output/aliyun-modelstudio-livetranslate/validate.txt` is generated.

## Output And Evidence

- Save translation session payloads and response summaries under `output/aliyun-modelstudio-livetranslate/`.
...
```bash
python skills/ai/audio/aliyun-modelstudio-livetranslate/scripts/prepare_livetranslate_request.py \
  --source-language zh \
  --target-language en \
  --output output/aliyun-modelstudio-livetranslate/request.json
```
