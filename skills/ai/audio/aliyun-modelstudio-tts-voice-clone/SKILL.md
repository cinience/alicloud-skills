---
name: aliyun-modelstudio-tts-voice-clone
...
```bash
.venv/bin/python skills/ai/audio/aliyun-modelstudio-tts-voice-clone/scripts/prepare_voice_clone_request.py \
...
```bash
mkdir -p output/aliyun-modelstudio-tts-voice-clone
for f in skills/ai/audio/aliyun-modelstudio-tts-voice-clone/scripts/*.py; do
  python3 -m py_compile "$f"
done
echo "py_compile_ok" > output/aliyun-modelstudio-tts-voice-clone/validate.txt
```

Pass criteria: command exits 0 and `output/aliyun-modelstudio-tts-voice-clone/validate.txt` is generated.

## Output And Evidence

- Save artifacts, command outputs, and API response summaries under `output/aliyun-modelstudio-tts-voice-clone/`.
